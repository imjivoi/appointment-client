export function useAnimateHeight({ parentRef }: { parentRef: HTMLFormElement }) {
  return ({ duration, easeFn, childRef }: { duration: number; easeFn: Function; childRef: HTMLFormElement }) => {
...
function animateValue(start = 0, end = 0, duration: number, easeFn: Function, parentRef: HTMLFormElement) {
